<template>
  ...
  >
    ...
    <a-upload-dragger v-if="drag" v-bind="newAttrs" v-on="newListeners">
      ...
      <p class="ant-upload-text">点击或拖拽到这里上传</p>
...
export default {
  ...
  methods: {
    ...
    async onCustomUpload(file) {
      ...
      return request
        .post(this.actionUrl, formData, { withCredentials: true })
        .then((res) => res.data)
        .then((res) => {
          // 成功之后直接调用成功的回调
          if (res.code === 0) {
...
.h-upload-box-wrap {
  ...
  .h-auto-scroll-upload-wrap /deep/ .ant-upload-list.ant-upload-list-text {
    height: 100%;
    max-height: 300px;
    overflow: auto;
  }
