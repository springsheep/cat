...
export default {
  ...
  data() {
    return {
      ...
      className: `dropdown-${new Date().getTime()}-${Math.floor(
  ...
  created() {
    if (this.$root.$el)
      this.$root.$el.addEventListener("click", this.onCloseDropdown);
  },
  beforeDestroy() {
    if (this.$root.$el)
      this.$root.$el.removeEventListener("click", this.onCloseDropdown);
  },
  methods: {
    onFocus(event) {
    ...
    onCloseDropdown(e) {
      ...
      if (
        ...
        !this.$refs.yearPicker.$el.contains(e.target)
      ) {
        this.showDropdown = false;
      }
    },
