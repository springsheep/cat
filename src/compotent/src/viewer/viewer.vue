  ...
  <div class="h-viewer-wrap">
    ...
    <div :class="`preview-${listType}-list`" v-if="showFileList">
      <template v-if="listType === 'link'">
        ...
        >
          <h-auto-tooltip :text="item.name"></h-auto-tooltip>
        </a>
      </template>
      <template v-else-if="listType === 'custom'">
        <div
          class="custom-item-box"
          v-for="(item, index) in records"
          :key="item.uid"
      ...
      <template v-else>
        <div
          v-for="(item, index) in records"
          :key="item.uid"
          :class="['list-item', `list-${listType}-item`]"
          :style="cardStyle"
          @click="onPreview(item, index)"
        >
          <img
    ...
    <transition name="scale" @after-leave="() => onAfterClosed()">
      <div
        v-if="visible"
        ref="viewerModal"
        :class="['h-viewer-modal-wrap', { 'h-viewer-modal-drag': drag }]"
        :style="viewerWrapStyle"
        @mousewheel="onScrollScale"
      >
        <div
          class="viewer-drag-bar"
          v-if="drag"
          @mousedown.stop.prevent="onBarMouseDown"
        />
        <a-icon
          type="close"
          class="icon-fixed icon-close"
          @click="onHideModal"
        />
        <div class="preview-content">
          <img
            v-if="record.fileType === 'image'"
            class="item-image"
            :key="record.url"
            :style="imgStyle"
            :src="record.url"
          ...
          />
          <video
            v-if="record.fileType === 'video'"
            class="item-video"
            controls
            :src="record.url"
          />
          <audio
            v-if="record.fileType === 'audio'"
            class="item-audio"
            controls
            :src="record.url"
          />
          <template v-if="record.fileType === 'pdf'">
            <h-pdf-canvas
              v-if="usePdf"
              :url="record.url"
              v-bind="typeof usePdf === 'boolean' ? null : usePdf"
            />
          ...
          <div class="preview-image-action">
            ...
            <a-icon
              type="undo"
              @click="onRotateLeftChange"
              :disabled="record.fileType !== 'image'"
            />
            <a-icon
              type="redo"
              @click="onRotateRightChange"
              :disabled="record.fileType !== 'image'"
            />
            <a-icon
              type="zoom-in"
              @click="onScaleAddChange"
              :disabled="record.fileType !== 'image'"
            />
            <a-icon
              type="zoom-out"
              @click="onScaleLessChange"
              :disabled="record.fileType !== 'image'"
            />
            <a-icon
              v-if="showDownload"
            ...
            <a-icon
              type="left"
              @click="onFileRightSwitch"
              :disabled="activeRecordIndex <= 0"
            />
            <a-icon
              type="right"
              @click="onFileLeftSwitch"
        ...
        <div class="preview-list-navbar">
          ...
          <div class="preview-nav-list">
            <div class="preview-nav-main" :style="toolbarStyle">
              <div
                :class="[
                  'list-item',
                  { 'list-item-active': item.uid === record.uid },
                ]"
                v-for="(item, index) in records"
                :key="item.uid"
                @click="onFileChange(item, index)"
              >
                <img
                  v-if="item.fileType === 'image'"
                  class="item-image"
          ...
          <a-icon
            type="left"
            class="icon-switch icon-left"
            v-if="totalPage > 0"
            @click="onToolbarScrollRight"
            :disabled="currentPage <= 0"
          />
          <a-icon
            type="right"
            class="icon-switch icon-right"
            v-if="totalPage > 0"
            @click="onToolbarScrollLeft"
...
<script>
import HAutoTooltip from "../auto-tooltip";
...
export default {
  ...
  props: {
    ...
    listType: {
      ...
      default: "thumbnail",
      validator: (value) =>
        ["thumbnail", "link", "card", "custom"].indexOf(value) > -1,
    },
    /**
  ...
  computed: {
    ...
    toolbarStyle() {
      return {
        justifyContent:
          this.totalWidth < this.mainWidth ? "center" : "flex-start",
  ...
  methods: {
    ...
    getFileUrl(url) {
      ...
      if (this.addDownloadPath) {
        return this.addTimestamp(
          (this.downloadUrl || this.$api.download || "") + url
        );
      }
      return this.addTimestamp(url);
    },
    ...
    getFileType(types = []) {
      ...
      if (types.find((t) => t.match(/\.(png|jpg|jpeg|gif|svg)(\?.*)*?$/i))) {
        ...
        types.find((t) => t.match(/\.(mp4|avi|mov|wmv|flv|webm|ogg)(\?.*)*?$/i))
      ) {
        return "video";
      } else if (
        types.find((t) =>
          t.match(/\.(mp3|cd|m4a|wma|wav|ape|flac|ogg|aac|vqf)(\?.*)*?$/i)
        )
      ) {
        return "audio";
