...
export default {
  ...
  data() {
    ...
    if (this.showPagination) {
      localPagination = {
        ...
        pageSize: this.pageSize || (this.$HIT || {}).pageSize || 10,
        pageSizeOptions,
  ...
  computed: {
    ...
    localPageSize() {
      return this.pageSize || (this.$HIT || {}).pageSize || 10
    },
  },
  created() {
        ...
        const resizeHeader = (h, props, children) => {
          ...
          const onDragStop = (x, y) => {
            ...
            if (this.columnStorage && this.$utils && this.$utils.setStorage && typeof window !== 'undefined') {
              this.$utils.setStorage(this.$route.path, {
      ...
      const needTotalItems = this.needTotalList.map((item) => {
        return (
          <span style="margin-right: 12px">
            {item.title} 总计{' '}
            <a style="font-weight: bold">{item.customRender ? item.customRender(item.total) : item.total}</a>
          </span>
        )
      })
      return (
        <a-alert showIcon={true}>
          <template slot="message">
            <span style="margin-right: 12px">
              已选择：
              <a style="font-weight: bold">{this.selectedRows.length}</a>
            </span>
            {needTotalItems}
            {this.selectedRows.length ? (
              <a style="margin-left: 24px" onClick={this.onClearSelected}>
                清空
              </a>
            ) : null}
          </template>
  ...
  render() {
    return (
      <div class="h-table-wrapper">
        ...
        <a-table class="h-table" {...this.newProps}>
