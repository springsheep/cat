...
import { getRoleList, getServiceList, user, commentd, user1 } from '@/api/manage'
import { commentquery } from '@/api/login'
const columns = [
  {
    title: '评论内容',
    dataIndex: 'commentContext',
  },
  {
    title: '评论人',
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return commentquery(requestParameters).then((res) => {
          return res.body
        })
      },
      selectedRowKeys: [],
      ...
      formOptions: [
        {
          label: '评论内容', // label文字
          prop: 'commentContext', // 字段名
          type: 'input', // 指定antd组件
          defaultValue: '', // 字段初始值
          placeholder: '请输入评论内容', // antd组件属性
