 ...
 * @LastEditors: 张鹏
 * @LastEditTime: 2022-05-08 19:50:17
-->
<template>
  <div>
    ...
    <a-list-item :key="item.id" slot="renderItem" slot-scope="item">
      <template slot="actions">
          ...
          <a @click="shoucang(item, item.favorite)" > {{ !item.favorite ? '收藏' : '取消收藏' }}</a>
...
import { shoucang, quxiaoshoucang,article, artd,articleadd1 } from '../api/login'
export default {
  name: 'Article',
  components: {
    IconText,
  ...
  methods: {
    ...
    getList(e={}) {
      article({pageSize: 10, pageNo: this.current, isArticle: false ,...e}).then((res) => {
