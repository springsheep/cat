...
import { getSmsCaptcha, get2step, newlogin, current } from '@/api/login'
...
export default {
  ...
  methods: {
    ...
    handleSubmit(e) {
      ...
      validateFields(validateFieldsKey, { force: true }, (err, values) => {
        if (!err) {
          ...
          loginParams.userPassword = values.password
          newlogin(loginParams)
            .then((res) => res.body && this.loginSuccess(res))
    ...
    loginSuccess(res) {
      ...
      current().then((res1) => {
        localStorage.setItem('userRole', res1.body.userRole)
        localStorage.setItem('userName', res1.body.userName)
        localStorage.setItem('id', res1.body.id)
